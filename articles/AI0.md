...
先停一下。你剛剛卡住的那個地方，不是你笨。**整個領域在那裡卡了十七年。**

1958 年感知器問世的時候場面相當浮誇。紐約時報跑去報導，說這台機器以後會走路、會說話、
會自我複製、會有自我意識。海軍出錢，經費滾滾而來。

然後 1969 年，Minsky 和 Papert 出了一本書叫《Perceptrons》，
用數學證明了一件很掃興的事：**單層感知器連 XOR 都做不到。** <sub>（蛤?你說你不知道XOR是什麼?，再往下捲一點就會看到的那張四列的表。）<sub>

而在書中提到多層網路應該有辦法，但緊接著補了一句:
沒有人知道要怎麼訓練它。

這個問題有正式名稱，叫**信用分配問題（credit assignment problem）**：

> 最後的答案錯了 0.5。可是這 0.5 是一整條路徑上好幾十顆神經元**一起**弄出來的。
> 那麼**這筆帳要怎麼算？**

算不出來，中間層不能更新，疊再多層都只是裝飾品。

於是錢停了、人走了。這段時間後來被稱為**第一次 AI 寒冬**(只是其中一個原因)。
...
1986 年，Rumelhart、Hinton 和 Williams 在《Nature》上發了一篇論文，
把一個當時已經被不同人各自重新發明過好幾次的東西講清楚，並且推到了整個領域面前。

那個東西叫**反向傳播（backpropagation）**。

它厲害的地方不是什麼新數學而在於:**它換掉了問題**
...
> 中間那顆神經元的輸出**動一點點**，最後的 loss 會**變多少**？

看出差別了嗎？

這個動一點點是一個可以算出來的變化量，

第一個問題要的是「標準答案」——你沒有，也不可能有。
第二個問題要的是「**斜率**」——而斜率**算得出來**。

而且第二個問題就已經夠用了。回頭想想梯度下降在幹嘛：
它從頭到尾**不需要知道正確答案**，它只需要知道**往哪邊走 loss 會變小**。

> **這顆神經元該吐什麼 不知道沒關係。
> 這顆神經元往上調一點會不會比較好 知道就夠了。**

這句話就是深度學習那道鴻溝被跨過去的地方:反向傳播。

### 具體怎麼做的?

不用碰偏微分<sub>碰了會想死，真的<sub>，
我們用最小的例子——一個輸入、一個權重：
...
這就是**連鎖律（chain rule）**。名字很唬人，但它做的事只有一件：
**把沿路每一段的 倍率 一路乘起來。**

而網路的前向計算(前向傳播)本來就是一條有順序的鏈：

$$
x \;\to\; z_1 \;\to\; a_1 \;\to\; z_2 \;\to\; a_2 \;\to\; \text{out} \;\to\; L
$$

既然是一條鏈，那就可以**從尾巴倒著走回來**，一段一段把倍率乘上去。
...
我們拿一個小到可以用手驗算的網路：1 個輸入 → 2 顆中間神經元 → 1 個輸出。

然後做兩件事：一次用反向傳播算中間層的責任，一次**手動給中間層一個極小的擾動**，看看 loss 會變多少。
...
W1 = np.array([[0.5, -0.3]]); b1 = np.array([0.1, 0.2])
W2 = np.array([[0.7], [1.2]]); b2 = np.array([-0.4])

def forward(bump=None):
    ...
    out = h @ W2 + b2
    return h, out, float((out - y)[0, 0] ** 2)

h, out, loss = forward()

print(
    "中間層輸出 h =", h[0],
    " 最後輸出 =", round(float(out[0, 0]), 4),
    " loss =", round(loss, 4)
)

# 反向傳播說
d_out = 2 * (out - y)            # loss 對 out 的斜率
d_h   = d_out @ W2.T             # 往前傳一層 ← 這一行就是反向傳播

print("反向傳播算的 :", np.round(d_h[0], 6))

# 給中間層某一顆一個極小的擾動，看 loss 改變多少
eps = 1e-6
...
for i in range(2):
    ...
    _, _, loss2 = forward(bump)

    brute.append((loss2 - loss) / eps)

print("數值微分算的 :", np.round(brute, 6))
```

```text
中間層輸出 h = [ 1.1 -0.4]  最後輸出 = -0.11  loss = 1.2321

反向傳播算的 : [-1.554 -2.664]

數值微分算的 : [-1.554    -2.663999]
```
...
> 給某個中間神經元一個極小的**擾動**，觀察 loss 改變多少。

這就是在用數值方法估計它的斜率。

兩種方法最後得到幾乎一樣的結果——一種是反向傳播**直接算出梯度**，另一種則是靠**微小擾動後觀察變化**來估計梯度。

停下來看清楚這件事的份量：

我**從頭到尾沒有告訴程式「中間層的正確答案是多少」**——因為根本沒有這種東西。

但 `d_out @ W2.T` 這一行，替中間層那兩顆神經元各算出了一個數字：`-1.554` 和 `-2.664`。

這兩個數字的意思是：

> **在目前的位置附近，中間層的輸出增加一點點，loss 大約會改變多少。**

有了這個數字，中間層就更新得動了。

原本「不知道中間層該怎麼調」的鴻溝，就是這樣被填平的。

順帶解釋一下 `@ W2.T` 為什麼是「往前傳一層」：

前向的時候走的是 `h @ W2`（中間層 → 輸出層），

反向就把同一個矩陣轉過來走回去（輸出層 → 中間層）。

**同一條路，反著走。**

### 為什麼這是打開深度學習的大門，而不只是個小技巧

你可能會想：既然「施加一個小擾動、看 loss 變多少」也算得出梯度，那幹嘛還要推公式？直接擾動不就好了？

因為**擾動不起**。

每次只能估計**一個變數**的斜率。有幾個參數，就得分別擾動幾次，

而每擾動一次，都得把整個網路**重跑一遍**。


反向傳播不一樣：**前向跑一次、反向跑一次，全部參數的斜率一次到手。**
跟參數有幾個**完全無關**。<sub>他甚至是自動的<sub>

拿後面會用到的那個 1→32→32→1 小網路實測，它只有 1153 個參數，不算大：
...
params = []
for n_in, n_out in [(1, 32), (32, 32), (32, 1)]:
    params += [rng.normal(0, np.sqrt(2/n_in), (n_in, n_out)), np.zeros(n_out)]
print("這個網路的參數量 =", sum(p.size for p in params))

def forward(ps):
    W1, b1, W2, b2, W3, b3 = ps
    a1 = relu(X @ W1 + b1)
    a2 = relu(a1 @ W2 + b2)
    return np.mean((a2 @ W3 + b3 - Y) ** 2)

# 數值梯度，一個參數改一次
t0 = time.perf_counter()
eps = 1e-6
base = forward(params); n_forward = 1
for p in params:
    flat = p.ravel()
    for i in range(flat.size):
        old = flat[i]
        flat[i] = old + eps
        forward(params); n_forward += 1
        flat[i] = old
...
dz2 = (d_out @ W3.T) * relu_grad(z2)
dW2 = a1.T @ dz2;   db2 = dz2.sum(0)
dz1 = (dz2 @ W2.T) * relu_grad(z1)
dW1 = X.T @ dz1;    db1 = dz1.sum(0)
t_bp = time.perf_counter() - t0

print(f"數值梯度 : 前向 {n_forward} 次, 花了 {t_num*1000:.1f} ms")
print(f"反向傳播 : 前向 1 次 + 反向 1 次, 花了 {t_bp*1000:.2f} ms")
print(f"快了 {t_num/t_bp:.0f} 倍")
```

```
這個網路的參數量 = 1153
數值梯度 : 前向 1154 次, 花了 76.1 ms
反向傳播 : 前向 1 次 + 反向 1 次, 花了 0.34 ms
快了 225 倍
```

<sub>（時間會因為你的機器不同而不同，但那個 1154 次 vs 1 次 是固定的）</sub>

**1153 個參數就差 200 倍。**

而且這個差距會隨著模型變大**線性擴大**參數變 10 倍，數值梯度就慢 10 倍，
數值梯度：
每一個參數 ≈ 一次 forward : O(P × C_forward)

反向傳播：
一次 forward + 一次 backward: O(C_forward)
<sub>什麼? 你不認識O()? 那就去學<sub>

把數字放大到現實：GPT-3 有約 1750 億個參數。
用擾動的，算**一次**梯度要跑 1750 億次前向。
用反向傳播，一次前向加一次反向就結束了。

> **所以反向傳播不是 算梯度的一個比較快的方法。
> 它是 深度網路到底訓不訓練得動 這件事本身的前提。**
>
> 沒有它，你可以疊一百層，但你永遠訓練不動它。

這就是為什麼 1986 年之後，「**深**」才第一次變成一個可以認真考慮的選項。

### 先記住三個動作

接下來每一份程式碼裡都會出現這三種東西，先認個臉：

| 你會看到 | 它在幹嘛 |
|:---|:---|
| `d_` 開頭的變數 | 「這個東西要負多少責任」，也就是 loss 對它的斜率 |
| `@ W.T`（乘上轉置） | 把責任**往前傳一層**（同一條路反著走） |
| `* 激活函數的斜率` | 「這顆當時有沒有出力？沒出力就不用負責」 |

認得這三個，下面每一坨反向傳播的程式碼你都讀得懂。

---

## 回到疊層：但還有第二道關卡

誤差現在傳得回去了。但**反向傳播有個前提：沿路每一段都要有斜率可以乘。**

而我們的網路裡卡了一個階梯函數。它只會講「0」或「1」——

我們前面才花一整節在講這件事：**只知道對錯、不知道差多遠，就沒辦法算斜率。**
現在問題更嚴重了，因為誤差不只要算，還要**穿過中間好幾層傳回去**。
階梯函數的斜率除了跳的那一瞬間以外**整片都是 0**——
連鎖律一乘到它就全部歸零，責任在那一層就斷了，什麼都傳不到前面去。
...
for step in range(3001):
    ...
    # 反向：把誤差一層一層傳回去
    d_out = 2 * (out - Y) / len(X) # 這個 2 就是剛剛連鎖律那段的 d(u^2)/du = 2u
...
現在你應該讀得懂那一大坨 `d_` 開頭的東西了——它就是前面那個**反向傳播**，
只是這次一次疊了三層：

| 這一行 | 在做什麼 |
|:---|:---|
| `d_out = 2 * (out - Y) / len(X)` | 起點：loss 對輸出的斜率 |
| `dW3 = a2.T @ d_out` | 最後一層的權重要負多少責任 → 拿去更新 |
| `da2 = d_out @ W3.T` | **把責任往前傳一層**（同一條路反著走） |
| `dz2 = da2 * relu_grad(z2)` | 當時沒被激活的那些神經元沒出力，不用負責 |
| `dW2 = a1.T @ dz2` | 第二層的權重要負多少責任 → 拿去更新 |
| ……一路重複到第一層 | |

`d_out` → `da2` → `dz2` → `da1` → `dz1` 這條路，就是連鎖律在**一路乘回去**，
只是每一段的倍率從一個數字換成了矩陣。

**這就是全部了。** 沒有第二套機制——三十層、三百層也是同一套，只是這幾行多重複幾次而已。

下面再給你兩個工具，讓你自己手推的時候不會死。

### 工具一：不信？暴力算給你看
整篇的風格就是 不信就算給你看 ，梯度也不例外。<sub>因為這樣我就理由丟給AI算了ˋwˊ<sub>

前面驗中間層責任的時候，我們用過「擾動一下看 loss 變多少」這一招。
它不只能驗中間層，**驗權重的梯度也一樣好用**。

拿最小的例子：上面推出 $\frac{\partial L}{\partial w} = 2(\hat y - y)x$，
那就**把 $w$ 加一個超小的數，看 $L$ 變了多少，再除以那個超小的數**，兩邊對一下。
...
<sub>（提醒一下：它是**驗算工具**，不是訓練工具。原因前面算過了 一個參數要戳一次，拿來訓練會慢到天荒地老。）</sub>
...
flowchart TD
    ...
    D -->|"那縱向呢?"| K["中間層沒有標準答案<br/>信用分配問題"]
    K -->|"改問責任<br/>連鎖律一路乘回去"| BP["反向傳播<br/>深度終於訓練得動"]
    BP -->|"但階梯的斜率整片是 0<br/>先拿掉試試"| E["多層線性堆疊<br/>疊三層"]
...
- 中間層卡在哪？——資料裡沒有它的標準答案，這叫信用分配問題，整個領域卡了十七年
- 反向傳播怎麼繞過去的？——不問「它該吐什麼」，改問「它動一點 loss 會變多少」；後者算得出來
- 反向傳播實際在算什麼？——把沿路每一段的倍率一路乘起來，算出每個權重該負多少責任
- 那為什麼不用數值梯度就好？——一個參數要戳一次；反向傳播一次前向加一次反向就全拿到
- 為什麼說它「打開了深度的大門」？——沒有它，你疊得起一百層，但永遠訓練不動
- 為什麼一定要激活函數？——沒有它，疊幾層都等於一層（而且你親眼看過 `[0.5 0.5 0.5 0.5]`）
- 為什麼是 ReLU 而不是階梯？——階梯斜率整片是 0，連鎖律一乘就歸零；ReLU 有折角又有斜率
